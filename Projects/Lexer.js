function lex(userInput){
    ...
    */
    ...
    Unimportant now and probably unimportant for the future, felt cute might delete later
    ...
    */
    ...
    Create a variable that separates the users input
    Get the total number of numLines that the user submitted and separate by commas
    Convert it to a string so we can actually split it
    */
    ...
    for(var currentLine = 0; currentLine < numLines.length; currentLine++){
        ...
        for(var currentColumn = 0; currentColumn < line.length; currentColumn++){
            ...
            */
            //Create variable that tracks whether we are in a comment or not
            var stillInComment;
            //If that variable is true then we are in a comment and must
            //Just continue lexing, skipping until we see the end of a comment
            if(stillInComment){
                continue;
            }

            /*If the currentToken we are lexing is a '/' we know the
            only correct character that could follow is a '*'
            */
            if(currentToken == "/" && line[currentColumn + 1] == "*"){
                //Set stillInComment to true so we can skip commented characters
                stillInComment = true;
                //Account for already looked at '*' character
                currentColumn++;
                //Continue lexing
                continue;
            }
            ...
            */
            if(currentToken == "*" && line[currentColumn + 1] == "/"){
                //Set stillInComment to false so we can stop skipping characters
            ...
            */
            if(currentToken == "b"){
                if(line[currentColumn + 1] == "o" && line[currentColumn + 2] == "o"
                   && line[currentColumn + 3] == "l" && line[currentColumn + 4] == "e" 
                   && line[currentColumn + 5] == "a" && line[currentColumn + 6] == "n"){
